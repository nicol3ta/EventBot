...
intents.matches('welcome', [
    function (session) { 
     ...
     function (session, results) { 
         ...
         session.send("Nice, %s is a cool city ;)", session.userData.city); 
         if(!oauth_token){
            var clientAddress = JSON.parse(session.message); 
            session.endDialog("You can ask me anything regarding meetups. \n But first please sign in:"  + JSON.stringify(clientAddress.address[0].serviceUrl) +"  URI: " , redirectUri+""+JSON.stringify(clientAddress.address[0].serviceUrl));  
         }
         else{
             session.endDialog("You can ask me anything regarding meetups."); 
         }
